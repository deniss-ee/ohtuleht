...
function resizeArtboardByHeight(doc, targetHeightMM) {
    ...
    if (Math.abs(scaleRatio - 1.0) < CONFIG.SCALING_PRECISION) {
        alert('Artboard height already matches target dimension.\nNo scaling needed.');
        return;
    }
...
function main() {
    ...
    if (app.documents.length === 0) {
        alert(
            'No document is open.\n\n' +
            'Please open an Illustrator document before running this script.'
        );
        return;
    }
    ...
    if (!dimensions) {
        alert(
            'Could not find dimensions in filename.\n\n' +
            'Expected format: WIDTHxHEIGHT (in millimeters)\n' +
            'Examples:\n' +
            '  • poster_210x297.ai (A4 size)\n' +
            '  • banner_1200x300.ai\n' +
            '  • card_85X55.ai\n\n' +
            'Current filename: ' + fileName + '\n\n' +
            'This script will resize by HEIGHT (second number).'
        );
        return;
    }
    ...
    if (!validateDimensions(dimensions.width, dimensions.height)) {
        alert(
            'Invalid dimensions: ' + dimensions.width + 'mm × ' + dimensions.height + 'mm\n\n' +
            'Dimensions must be between ' + CONFIG.MIN_DIMENSION + 'mm and ' + CONFIG.MAX_DIMENSION + 'mm'
        );
        return;
    }
    ...
    var confirmMsg = 
        'RESIZE BY HEIGHT\n\n' +
        'Target HEIGHT: ' + dimensions.height + 'mm\n' +
        'Width will adjust proportionally\n\n' +
        'This will scale the artboard and ALL objects.\n' +
