...
/**
 * Resize Artboard by Width (Scale by Width, Set Height)
 * 
 * Automatically scales all content based on WIDTH dimension, then sets artboard
 * height to exact value from filename without scaling height.
 * 
 * Usage:
 *   1. Name your file with format: "name_WIDTHxHEIGHT.ai" (dimensions in mm)
 *      Example: "banner_1200x300.ai"
 *   2. Run this script
 *   3. Content scales uniformly to match WIDTH (1200mm)
 *      Artboard height sets to exact HEIGHT (300mm) without additional scaling
 * 
 * Features:
 *   - Scales ALL objects uniformly by WIDTH ratio (maintains aspect ratio)
 *   - Sets artboard HEIGHT to exact filename value (no height scaling)
...
/**
 * Resizes artboard by scaling content to match WIDTH, then adjusts height
 * Objects scale uniformly by width ratio, artboard height adjusts to filename value
 * 
 * @param {Document} doc - Active document
 * @param {number} targetWidthMM - Target width in millimeters (scales content)
 * @param {number} targetHeightMM - Target artboard height in millimeters (no scaling)
 */
function resizeArtboardByWidth(doc, targetWidthMM, targetHeightMM) {
    ...
    // Calculate scale ratio based on WIDTH only (uniform scaling)
    ...
    // Scale all objects uniformly by WIDTH ratio
    ...
    // Finally, adjust artboard height to exact target (without scaling objects)
    artboard.artboardRect = [
        dims.left,
        dims.top,
        dims.left + newWidthPt,
        dims.top - targetHeightPt  // Set to exact height from filename
    ];
